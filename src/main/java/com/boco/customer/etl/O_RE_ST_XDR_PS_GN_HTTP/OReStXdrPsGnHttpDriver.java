...
public class OReStXdrPsGnHttpDriver {


    static final String NAME = "GnHttpDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsGnHttpDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("gnhttp.name");
	String family = pro.getProperty("gnhttp.family");
	String column = pro.getProperty("gnhttp.column");
	String keyLen = pro.getProperty("gnhttp.key.length");
	...
	if_index = pro.getProperty("gnhttp.if.index");
	// 设置hbase参数
	...
	job.setJarByClass(OReStXdrPsGnHttpDriver.class);
	...
	job.setMapperClass(OReStXdrPsGnHttpMapper.class);
     ...
     * @param day
     */
    private static void checkTable(Configuration conf, String mainTable, String day) {
	try {
		...
		// 创建表并进行预分区
		Spliter.createGnHttpTable(conf,mainTable);
	    }
