...
public class OReStXdrPsS11Driver {


    static final String NAME = "S11Detail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS11Driver.class);
    // 配置文件
	...
	String table = pro.getProperty("s11.name");
	//增加索引表
	String index = pro.getProperty("s11.index");
	String family = pro.getProperty("s11.family");
	String column = pro.getProperty("s11.column");
	String keyLen = pro.getProperty("s11.key.length");
	if_index = pro.getProperty("s11.if.index");
	...
	job.setJarByClass(OReStXdrPsS11Mapper.class);
	...
	job.setMapperClass(OReStXdrPsS11Mapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createS11Table(mainTable);
	    }
