...
public class OReStXdrPsS1uMmeDriver {


    static final String NAME = "S1MmeDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uMmeDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("mme.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("mme.index");
	String family = pro.getProperty("mme.family");
	String column = pro.getProperty("mme.column");
	String keyLen = pro.getProperty("mme.key.length");
	String kerberos_count = pro.getProperty("kerberos_count");
	String if_kerberos = pro.getProperty("if_kerberos");
	if_index = pro.getProperty("mme.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uMmeMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uMmeMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createMmeTable(conf,mainTable);
	    }
