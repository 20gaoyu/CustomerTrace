package com.boco.customer.etl.O_RE_ST_XDR_PS_S1MME_CAUSE;
...
public class OReStXdrPsS1uMmeCauseDriver {


    static final String NAME = "S1MmeDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uMmeCauseDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("mmecause.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("mmecause.index");
	String family = pro.getProperty("mmecause.family");
	String column = pro.getProperty("mmecause.column");
	String keyLen = pro.getProperty("mmecause.key.length");
	if_index = pro.getProperty("mmecause.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uMmeCauseMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uMmeCauseMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createMmeTable(mainTable);
	    }
