...
import com.boco.customer.fieldJl.S1Mme;
...
public class OReStXdrPsS1uMmeCauseMapper extends Mapper<LongWritable, Text, NullWritable, NullWritable> implements S1Mme {
...
//		}
	 ...
	 // 业务处理
	   ...
	   // String region_id = xdrdata[CITY];
		...
		sb.append(FAILURE_CAUSE).append("|");
		...
		sb.append(load_time);
		// 组织key
		String cause_value=causeValFormat(xdrdata[FAILURE_CAUSE]);
		byte[] rowkey = KeyUtils.buildKey(cause_value, endTime_format.replaceAll("[- :]", ""));// 六位原因码+20160522152800+四位随机数+(int)(Math.random()*1000)
		// 向hbase中插入数据
