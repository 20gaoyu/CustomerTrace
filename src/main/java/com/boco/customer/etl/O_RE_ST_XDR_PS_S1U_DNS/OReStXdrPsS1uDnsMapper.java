...
public class OReStXdrPsS1uDnsMapper extends Mapper<LongWritable, Text,ImmutableBytesWritable, KeyValue> implements S1uDns {
...
//				indexTable.setWriteBufferSize(12 * 1024 * 1024);
		...
		dimXdrCause = HdfsToDim.getDimInfo(conf, "/user/boco/dim/DIM_XDR_CAUSE_DNS/", "0", valueDnsCau);
		//黑龙江的的小区维表和一般的有区别
		mDimNeEc = HdfsToDim.getDimInfo(conf, "/user/boco/dim/DIM_NE_EC/", "0", valueEc);
	...
	protected void map(LongWritable key, Text value, Context context) {
		...
		// 小区信息
			    ...
			    //黑龙江的做特殊处理
			    if(provinceName.equals("HEILONGJIANG")) {
			    if(xdrdata[ECI].length()>=7&&xdrdata[ECI].length()<=9)
				{
			    try {
				...
				// 获取原因码
				strDimXdrCause = dimXdrCause.get(xdrdata[DNS_RESP_CODE] + "");// CAUSE_VALUE
				if (StringUtils.isNotBlank(strDimXdrCause)) {
				    dimXdrCauseArr = pattern.split(strDimXdrCause + "|w");
		...
		sb.append(xdrdata[XDR_TYPE_ID]).append("|");
