package com.boco.customer.etl.O_RE_ST_XDR_PS_S1U_EMAIL;
...
public class OReStXdrPsS1uEmailDriver {


    static final String NAME = "S1uEmailDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uEmailDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("email.name");
	String index = pro.getProperty("email.index");
	String family = pro.getProperty("email.family");
	String column = pro.getProperty("email.column");
	String keyLen = pro.getProperty("email.key.length");
	String mainTable = table.replace("$DAY$", day);
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String indexTable = index.replace("$DAY$", day);
	if_index = pro.getProperty("email.if.index");
	// 设置hbase参数
	...
	job.setJarByClass(OReStXdrPsS1uEmailMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uEmailMapper.class);
