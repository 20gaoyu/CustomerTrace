...
public class OReStXdrPsS1uFtpDriver {


    static final String NAME = "S1uFtpDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uFtpDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("ftp.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("ftp.index");
	String family = pro.getProperty("ftp.family");
	String column = pro.getProperty("ftp.column");
	String keyLen = pro.getProperty("ftp.key.length");
	if_index = pro.getProperty("ftp.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uFtpMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uFtpMapper.class);
