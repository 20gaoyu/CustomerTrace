package com.boco.customer.etl.O_RE_ST_XDR_PS_S1U_GNRL;
...
public class OReStXdrPsS1uGnrlDriver {


    static final String NAME = "S1uGnrlDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uGnrlDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("gnrl.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("gnrl.index");
	String family = pro.getProperty("gnrl.family");
	String column = pro.getProperty("gnrl.column");
	String keyLen = pro.getProperty("gnrl.key.length");
	...
	if_index = pro.getProperty("gnrl.if.index");
	// 设置hbase参数
	...
	job.setJarByClass(OReStXdrPsS1uGnrlMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uGnrlMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createGnrlTable(mainTable);
	    }
