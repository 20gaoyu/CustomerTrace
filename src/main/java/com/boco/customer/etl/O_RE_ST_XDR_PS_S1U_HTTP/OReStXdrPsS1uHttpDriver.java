package com.boco.customer.etl.O_RE_ST_XDR_PS_S1U_HTTP;
...
public class OReStXdrPsS1uHttpDriver  {


    static final String NAME = "S1uHttpDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uHttpDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("http.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("http.index");
	String family = pro.getProperty("http.family");
	String column = pro.getProperty("http.column");
	String keyLen = pro.getProperty("http.key.length");
	...
	if_index = pro.getProperty("http.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uHttpMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uHttpMapper.class);
