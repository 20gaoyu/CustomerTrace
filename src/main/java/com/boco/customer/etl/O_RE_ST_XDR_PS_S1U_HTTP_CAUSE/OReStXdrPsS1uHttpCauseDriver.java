...
public class OReStXdrPsS1uHttpCauseDriver {


    static final String NAME = "S1uHttpCauseDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uHttpCauseDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("httpcause.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("httpcause.index");
	String family = pro.getProperty("httpcause.family");
	String column = pro.getProperty("httpcause.column");
	String keyLen = pro.getProperty("httpcause.key.length");
	...
	if_index = pro.getProperty("httpcause.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uHttpCauseMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uHttpCauseMapper.class);
