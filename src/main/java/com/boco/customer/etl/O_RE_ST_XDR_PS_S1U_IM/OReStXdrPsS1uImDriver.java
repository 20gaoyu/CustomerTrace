...
public class OReStXdrPsS1uImDriver {


    static final String NAME = "S1uImDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uImDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("im.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("im.index");
	String family = pro.getProperty("im.family");
	String column = pro.getProperty("im.column");
	String keyLen = pro.getProperty("im.key.length");
	String mainTable = table.replace("$DAY$", day);
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String indexTable = index.replace("$DAY$", day);
	if_index = pro.getProperty("im.if.index");
	// 设置hbase参数
	...
	job.setJarByClass(OReStXdrPsS1uImMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uImMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createImTable(mainTable);
	    }
