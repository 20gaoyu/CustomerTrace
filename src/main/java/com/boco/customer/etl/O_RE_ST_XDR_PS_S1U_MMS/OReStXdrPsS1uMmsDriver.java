...
public class OReStXdrPsS1uMmsDriver {


    static final String NAME = "S1uMmsDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uMmsDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("mms.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("mms.index");
	String family = pro.getProperty("mms.family");
	String column = pro.getProperty("mms.column");
	String keyLen = pro.getProperty("mms.key.length");
	if_index = pro.getProperty("mms.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uMmsMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uMmsMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createMmsTable(mainTable);
	    }
