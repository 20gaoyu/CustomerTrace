package com.boco.customer.etl.O_RE_ST_XDR_PS_S1U_P2P;
...
public class OReStXdrPsS1uP2pDriver {


    static final String NAME = "S1uP2pDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uP2pDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("p2p.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("p2p.index");
	String family = pro.getProperty("p2p.family");
	String column = pro.getProperty("p2p.column");
	String keyLen = pro.getProperty("p2p.key.length");
	if_index = pro.getProperty("p2p.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uP2pMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uP2pMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createP2pTable(mainTable);
	    }
