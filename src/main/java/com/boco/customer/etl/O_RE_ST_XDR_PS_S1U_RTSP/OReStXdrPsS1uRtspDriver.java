...
public class OReStXdrPsS1uRtspDriver {


    static final String NAME = "S1uRtspDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uRtspDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("rtsp.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("rtsp.index");
	String family = pro.getProperty("rtsp.family");
	String column = pro.getProperty("rtsp.column");
	String keyLen = pro.getProperty("rtsp.key.length");
	String mainTable = table.replace("$DAY$", day);
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String indexTable = index.replace("$DAY$", day);
	if_index = pro.getProperty("rtsp.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uRtspMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uRtspMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createRtspTable(mainTable);
	    }
