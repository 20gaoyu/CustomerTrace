package com.boco.customer.etl.O_RE_ST_XDR_PS_S1U_VOIP;
...
public class OReStXdrPsS1uVoipDriver {


    static final String NAME = "S1uVoipDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS1uVoipDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("voip.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("voip.index");
	String family = pro.getProperty("voip.family");
	String column = pro.getProperty("voip.column");
	String keyLen = pro.getProperty("voip.key.length");
	if_index = pro.getProperty("voip.if.index");
	...
	job.setJarByClass(OReStXdrPsS1uVoipMapper.class);
	...
	job.setMapperClass(OReStXdrPsS1uVoipMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createVoipTable(mainTable);
	    }
