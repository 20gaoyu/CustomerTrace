...
public class OReStXdrPsS6aDriver {


    static final String NAME = "S1S6aDetail";
    // 日志
    static final Log log = LogFactory.getLog(OReStXdrPsS6aDriver.class);
    // 配置文件
	...
	String table = pro.getProperty("s6a.name");
	//用来查询imsi的需要增加索引表，rowkey的值为主表的rowkey
	String index = pro.getProperty("s6a.index");
	String family = pro.getProperty("s6a.family");
	String column = pro.getProperty("s6a.column");
	String keyLen = pro.getProperty("s6a.key.length");
	if_index = pro.getProperty("s6a.if.index");
	...
	job.setJarByClass(OReStXdrPsS6aMapper.class);
	...
	job.setMapperClass(OReStXdrPsS6aMapper.class);
	...
	try {
		...
		// 创建表并进行预分区
		Spliter.createS6aTable(mainTable);
	    }
