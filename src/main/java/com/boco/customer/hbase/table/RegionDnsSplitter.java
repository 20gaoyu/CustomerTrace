...
public class RegionDnsSplitter {
	static final Log logger = LogFactory.getLog(RegionDnsSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("dns.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("dns.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("dns.family");
	// 压缩方式
	static String compression = pro.getProperty("dns.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("dns.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("dns.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("dns.key.length"));
