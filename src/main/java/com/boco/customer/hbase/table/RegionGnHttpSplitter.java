...
public class RegionGnHttpSplitter {
	static final Log logger = LogFactory.getLog(RegionGnHttpSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("gnhttp.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("gnhttp.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("gnhttp.family");
	// 压缩方式
	static String compression = pro.getProperty("gnhttp.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("gnhttp.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("gnhttp.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("gnhttp.key.length"));
