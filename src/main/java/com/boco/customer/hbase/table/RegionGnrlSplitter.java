...
public class RegionGnrlSplitter {
	static final Log logger = LogFactory.getLog(RegionGnrlSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("gnrl.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("gnrl.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("gnrl.family");
	// 压缩方式
	static String compression = pro.getProperty("gnrl.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("gnrl.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("gnrl.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("gnrl.key.length"));
