...
public class RegionIndexSplitter {
	static final Log logger = LogFactory.getLog(RegionIndexSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("index.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("index.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("index.family");
	// 压缩方式
	static String compression = pro.getProperty("index.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("index.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("index.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("index.key.length"));
