...
public class RegionMmeSplitter {
	static final Log logger = LogFactory.getLog(RegionMmeSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("mme.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("mme.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("mme.family");
	// 压缩方式
	static String compression = pro.getProperty("mme.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("mme.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("mme.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("mme.key.length"));
