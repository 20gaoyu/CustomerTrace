...
public class RegionS11Splitter {
	static final Log logger = LogFactory.getLog(RegionS11Splitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("s11.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("s11.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("s11.family");
	// 压缩方式
	static String compression = pro.getProperty("s11.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("s11.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("s11.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("s11.key.length"));
