...
public class RegionS6aSplitter {
	static final Log logger = LogFactory.getLog(RegionS6aSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("s6a.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("s6a.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("s6a.family");
	// 压缩方式
	static String compression = pro.getProperty("s6a.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("s6a.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("s6a.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("s6a.key.length"));
