...
public class RegionSgsSplitter {
	static final Log logger = LogFactory.getLog(RegionSgsSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("sgs.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("sgs.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("sgs.family");
	// 压缩方式
	static String compression = pro.getProperty("sgs.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("sgs.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("sgs.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("sgs.key.length"));
