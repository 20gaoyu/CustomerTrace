...
public class RegionlteSplitter {
	static final Log logger = LogFactory.getLog(RegionlteSplitter.class);
	// 获取配置文件参数
	...
	// 分发处理器类
	static String[] coprocessor_class = pro.getProperty("custrack.coprocessor.class").split(",");
	// 分发处理器地址
	static String[] coprocessor_path = pro.getProperty("custrack.coprocessor.path").split(",");
	// 获取列族
	static String famliy = pro.getProperty("custrack.family");
	// 压缩方式
	static String compression = pro.getProperty("custrack.compression");
	// 表描述方式
	static String tableDescriptor = pro.getProperty("custrack.descriptor");

	// Region数量
	static int sprangsize = Integer.parseInt(pro.getProperty("custrack.region.size"));
	// key的长度
	static int keylength = Integer.parseInt(pro.getProperty("custrack.key.length"));
	...
	public static void spliter(Configuration conf,String table) throws IOException, ParseException {
		// 配置HBase参数
		//Configuration conf = HBaseConfiguration.create();
		conf.set("hbase.zookeeper.quorum", quorum);
		conf.set("hbase.zookeeper.property.clientPort", clientPort);
